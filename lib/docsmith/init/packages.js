...
var toYaml = require('js-yaml').safeDump;
...
var install = function install(_ref, done) {
  ...
  async.reduce(Object.keys(packages).map(function (name) {
    return { name, pkg: packages[name] };
    ...
    npm.load({ save: false, progress: false, loglevel: verbose ? 'info' : 'silent' }, function (err) {
      ...
      npm.commands[link ? 'link' : 'install']([link ? name : pkg], function (err) {
        ...
        fs.copySync(path.join(repository, 'packages', name, 'node_modules', name), path.join(repository, 'packages', name));

        var exists = fs.pathExistsSync(path.join(repository, 'packages', name, './content.yml'));
        debug('>> Checking content.yml in package at ' + path.join(repository, 'packages', name, './content.yml') + ':', exists);
        // Skip if there is no content.yml in the installed package
        if (!exists) return callback();

        // Otherwise gather the content of the content.yml.
        var content = yaml(read(path.join(repository, 'packages', name, './content.yml'), 'utf8'));

        // copy the content.yml to the root of the content repo.
        var exists_root_content = fs.pathExistsSync(path.join(repository, 'content.yml'));

        var new_root_content = void 0;

        if (exists_root_content) {
          var root_content = yaml(read(path.join(repository, 'content.yml'), 'utf8'));
