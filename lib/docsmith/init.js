    ...
    if (exists) {
      ...
      promzard(file, { content, defaults }, function (err, responses) {
        ...
        packages.install({ packages: content.packages, repository: responses.repository, link, verbose }, function (err, installed) {
          ...
          // Deploy symlinks.
          installed.forEach(function (_ref2) {
            var name = _ref2.name,
                workspace = _ref2.content.workspace;

            Object.keys(workspace).reduce(function (acc, workspace) {
              return acc.includes(workspace.split('/')[0]) ? acc : [].concat(_toConsumableArray(acc), [workspace.split('/')[0]]);
            }, []).forEach(function (group) {
              try {
                fs.ensureSymlinkSync(path.join(responses.repository, 'packages', name, 'content'), path.join(current, group));
                console.log('Symlinked', path.join(responses.repository, 'packages', name, 'content') + ' to ' + path.join(current, group));
              } catch (e) {
                exit('\nError while creating synlink from ' + path.join(responses.repository, 'packages', name, 'content') + ' to ' + path.join(current, group), e);
              }
            });
          });
