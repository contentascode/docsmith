...
var settings = require('./docsmith/utils/settings');
  ...
  case 'travis':
    {
      ...
      if (TRUE) {
        ...
        create_travis_yml(gh_token).then(jekyll_config('../components/travis/_config.yml', '_config.yml')).then(npm_build('../components/travis/npm_build.yml', 'package.json')).then(function () {
...
function create_travis_yml(gh_token) {
  return new Promise(function (resolve, reject) {
    ...
    var travis_yml = yaml.safeLoad(fs.readFileSync(path.join(templates.path, '../components/travis/.travis.yml'), 'utf8'));
